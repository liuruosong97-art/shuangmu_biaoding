...
class StereoDataset(EasyDataset):
    ...
    def __getitem__(self, index):
        ...
        返回:
            sample: 字典，包含:
                - img1: 左图 Tensor (C, H, W)，float32
                - img2: 右图 Tensor (C, H, W)，float32
                - meta: 图像路径元信息
        ...
        if self.is_test:
            ...
            # ===== 关键: 处理灰度图 (结构光 IR 图像) =====
            # 检测灰度图: shape = (H, W) 而非 (H, W, 3)
            if len(img1.shape) == 2:
                # 将灰度图复制为 3 通道 RGB
                # (H, W) → (H, W, 1) → (H, W, 3)
                # 原理: BridgeDepth 在 RGB 数据上训练，需要 3 通道输入
                # 重复灰度值保持语义一致性
                img1 = np.tile(img1[..., None], (1, 1, 3))
                img2 = np.tile(img2[..., None], (1, 1, 3))
            else:
                # RGB 图像: 只取前 3 个通道 (忽略 alpha 通道)
                img1 = img1[..., :3]
                img2 = img2[..., :3]

            # ===== 转换为 PyTorch Tensor =====
            # NumPy (H, W, C) → Tensor (C, H, W)
            # permute(2, 0, 1): 将通道维度移到最前
            sample['img1'] = torch.from_numpy(img1).permute(2, 0, 1).float()
            sample['img2'] = torch.from_numpy(img2).permute(2, 0, 1).float()
            sample['meta'] = self.image_list[index][0]  # 文件路径
            return sample
