...
if __name__=="__main__":
  ...
  img0 = torch.as_tensor(img0, device=device).float()[None].permute(0,3,1,2)
  img1 = torch.as_tensor(img1, device=device).float()[None].permute(0,3,1,2)
  padder = InputPadder(img0.shape, divis_by=32, force_square=False)
  img0, img1 = padder.pad(img0, img1)

  autocast_ctx = torch.cuda.amp.autocast if device.type == 'cuda' else contextlib.nullcontext
  autocast_kwargs = {'enabled': True} if device.type == 'cuda' else {}
  ...
  if args.get_pc:
    ...
    try:
      ...
      if render_opt is None:
        raise RuntimeError("Open3D render option unavailable (headless environment?)")
      vis.add_geometry(pcd)
      render_opt.point_size = 1.0
      render_opt.background_color = np.array([0.5, 0.5, 0.5])
      vis.run()
    except Exception as exc:
      logging.warning(f"Skip point cloud visualization: {exc}")
    finally:
      vis.destroy_window()
