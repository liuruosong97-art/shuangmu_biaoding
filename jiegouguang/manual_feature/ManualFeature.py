...
class ManualFeatureJieGouGuang():
    """包含手动特征提取、匹配与点云生成流程的派生类。"""
    ...
    def extract_circle(self):
        centers_img1, img1_with_center, _ = self.extract_circle_subpixel(self.img1_rectify, method='gaussian')
        centers_img2, img2_with_center, _ = self.extract_circle_subpixel(self.img2_rectify, method='gaussian')
    ...
    def feature_matching(self, y_tol=1.0, mutual_check=True, ratio=1.0):
        ...
        min_disp = getattr(self, 'min_disp', None)
        max_disp = getattr(self, 'max_disp', None)
        if min_disp is None or max_disp is None:
            min_Z = getattr(self, 'max_dis', None)
            max_Z = getattr(self, 'min_dis', None)
            if min_Z is None or max_Z is None:
                raise ValueError('请先设置 min_dis/max_dis 或 min_disp/max_disp')
            min_disp = f * B / min_Z
            max_disp = f * B / max_Z
        ...
        for i in range(desc1.shape[0]):
            ...
            matches.append((i, j))
            dis_desc.append(float(d1))
            dis_spatial.append(disp)

        matches = np.array(matches)
        if matches.size == 0:
            raise ValueError('匹配失败，未找到有效匹配对')

        self.sift_matches = matches
        self.sift_dis_desc = np.array(dis_desc)
        self.sift_dis_spatial = np.array(dis_spatial)
    ...
    def pointcloud_from_disparity(self, output_ply=None, f=None, B=None, z_threshold=100, neighbor_radius=20):
        """通过视差计算深度并生成点云。"""
